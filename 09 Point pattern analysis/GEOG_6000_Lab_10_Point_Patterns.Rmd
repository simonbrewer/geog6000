---
title: "GEOG 6000 Lab 10 Spatial point patterns"
author: "Simon Brewer"
...
In this lab, we will carry out some point pattern data analysis. You will need three packages to carry out all the steps listed here: **rgdal**, **maptools** and **spatstat**, so make sure these are installed these before starting. The following files will be used:
...
\newpage
# Reading and plotting point process data
...
We will run this using a different point data set, the redwoods data: *redwood.shp*. Read this into R, and create a point process object. As the data is in a shape file, we will need to use the `readOGR()` function from the **rgdal** package, and we will also need the **maptools** package to convert these data to a `ppp` object, via a 'SpatialPoints' object. 
...
redwood.sp <- readOGR("./redwood/redwood.shp")
...
2. The function to be used (here Ripley's $K$)
...
In the previous sections, the point processes have been considered as single objects. *Marked* point process data include some information that distinguish the objects into different classes, allowing study of the co-occurrence (either positive or negative) between different classes of object. The Lansing forest data set contains objects that are labelled by species (6 marks). Read this file in and construct a point process object, with *marks* representing the different species. Some differences from before: (1) the window describing the study area is in a shape file (*lansing.shp* and will need to be read in and converted to an `owin` object; (2) we need to specify the class information (the species names) when creating the `ppp` object, using the `marks` parameter:

```{r fig.keep='none', message=FALSE, results='hide'}
lansing = read.csv("./lansing/lansing.csv")
lansing.win = readOGR("./lansing/lansing.shp")
...
Note that you will need to install and load the **maptools** package for this to work:
...
R Command & Purpose \\
\hline
\multicolumn{2}{|l|}{\textbf{Managing point process objects}} \\
\hline
\texttt{owin} & Create the observation window containing the points\\
\texttt{ppp} & Create a point pattern object\\
\texttt{split} & Split a marked point pattern object into individual marks\\
\hline
\multicolumn{2}{|l|}{\textbf{Analyzing point pattern data}} \\
\hline
\texttt{quadratcount} & Estimate the number of points in quadrats. Quadrats may be regular (parameters \texttt{nx} and \texttt{ny}) or defined as a set of spatial classes (parameter \texttt{tess})\\
\texttt{quadrat.test} & Test observed numbers of points in quadrats against expected numbers. Significance is inferred using a Chi-squared test\\
\texttt{tess} & Create a `tessellation' object representing the spatial extent of a set of classes\\
\texttt{density} & Fit Gaussian kernels to point pattern data. Parameter \texttt{sigma} defines the bandwidth as the 1SD size of the kernel (in spatial units)\\
\hline
\multicolumn{2}{|l|}{\textbf{Distance functions}} \\
\hline
\texttt{Kest} & Calculate Ripley's $K$ function\\
\texttt{plot.kest} & Plot Ripley's $K$ function, including the empirical function and theoretical functions\\
\texttt{Kcross} & Calculate Ripley's $K$ function from the distances between two marks of a point pattern dataset\\
\texttt{envelope} & Perform Monte Carlo simulation of spatially random point patterns with the same intensity as an observed set, and calculate a distance function for each pattern\\
\texttt{ppm} & Fit a point process model to a point pattern data set \\
\hline
\end{tabularx}
