...
This lab will introduce you to mixed effects (hierarchical) modeling and briefly to generalized additive modeling. It requires several additional libraries to be loaded: **nlme** and **lattice**, which are both part of the standard R installation, and **lme4**, which you will need to install for this lab. The following files will be used in this lab, all available on Canvas:

- The Math achievement dataset  *MathAch.csv*
- A dataset of introduced species   *speciesIntro.csv*
...
Hierarchical linear models allow you to extend the standard linear model to allow for variation in the model coefficients. So rather than having a single slope or intercept for the relationship between $y$ and $x_1$, we allow this to vary according between different groups of observations. In this way the model tries to follow a natural hierarchy in the data, where the global model can vary according to a higher level classification (the group membership). 
...
```{r results='hide'}
library(nlme)
math.lme1 <- lme(mathach ~ meanses*cses + sector*cses, random= ~ cses | school, data=MathAch)
summary(math.lme1)
```
...
The file *speciesIntro.csv* contains a simulated dataset related the success or failure of 60 species introduced into 4 different countries. The simple goal of this analysis is see if there is a difference in the overall success in different countries. As there are several different species per country, we include the intercept as a random effect across species.

```{r fig.keep='none'}
sppint  = read.csv("speciesIntro.csv")
plot(as.factor(success) ~ location, data=sppint)
```

## Model

As the success variable is binary (0/1), we use the `glmer()` function from the **lme4**. This uses the same arguments as the `glm()` models we looked at earlier this semester, with a `family` parameter to specify the distribution of the dependent variable. As this is a binary variable, we specify it as a two-column matrix (success and failure) in the model, using `cbind()`.
...
```{r}
library(lme4)
spp.glmm.1 = glmer(cbind(success, failure) ~ location + (1|species), 
                   family = binomial, data=sppint)
```
...
```{r results='hide'}
spp.glmm.2 = glmer(cbind(success, failure) ~ 1 + (1|species), family = binomial, data=sppint)
...
```{r fig.keep='none'}
plot(ozone.gam, resid=T, pch=16, select=1)
```
...
```{r fig.keep='none'}
vis.gam(ozone.gam, theta=230, phi=20)
```
...
plot(timeIDX, ozone$ozone, type="l", 
     xlab="Time", ylab="Ozone Conc ppb")
lines(timeIDX, fitted(ozone.gam), col=2)
legend("topleft",legend=c("Obs.","Est."),
       lty=1, col=c(1,2))
```
