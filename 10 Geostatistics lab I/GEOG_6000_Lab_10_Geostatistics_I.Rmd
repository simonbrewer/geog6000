...
You will need two packages to carry out all the steps listed here: **sf** and **gstat**, so make sure these are installed these before starting. 
...
For the Swiss data, there are three files: 

- the precipitation data at individual stations: *swiss_ppt.csv*
- a digital elevation model for Switzerland: *swiss_dem.grd*, which we will use for the prediction grid
- a shapefile of the Swiss border: *borders.shp*. 

Start by loading the **sf** package to get the functions for reading files, then load each data set and convert to the relevant Spatial* object. 

```{r message=FALSE, results='hide'}
library(sf)
swiss <- read.csv("../datafiles/swiss_ppt.csv")
...
# DEM for Switzerland
swiss.dem <- raster::raster("../datafiles/swiss_dem.asc")
