...
math.lme1 <- lme(mathach ~ meanses * cses + sector * cses, 
                 random = ~ cses | school, data = MathAch)
summary(math.lme1)
```
...
The file *speciesIntro.csv* contains a simulated dataset related the success or failure of 60 species introduced into 4 different countries. The simple goal of this analysis is see if there is a difference in the overall success in different countries. As there are several different species per country, we include the intercept as a random effect across species. First load and plot the data. The plot we use here is a *mosaic* plot. This shows the data as bars that are proportional to the number of `0`s and `1`s at each location. In this case, Australia has the lowest overall proportion of `1`s (successful introductions).

```{r fig.keep='none'}
sppint <- read.csv("../datafiles/speciesIntro.csv")
mosaicplot(location ~ success, data = sppint)
```
...
```{r results='markup'}
spp.glmm.2 <- glmer(cbind(success, failure) ~ 1 + (1|species), 
                    family = binomial, data = sppint)
