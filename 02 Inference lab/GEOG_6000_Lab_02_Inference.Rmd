...
In this lab, we will look at using R for statistical inference tests. We will be using the following files for these examples:
...
```
This will write a new csv file called *irished2.csv* in your working directory, containing the irished data frame, with the new column. Note that R uses the variable names as column headers and adds a column with line numbers. You can remove the line numbers by adding the parameter `row.names=FALSE` to the `write.csv()` function. 
...
```{r results='hide'}
irished$fsex <- factor(irished$sex, levels = c(1, 2),
    labels = c("male", "female"))
irished$flvcert <- factor(irished$lvcert, levels = c(0, 1), 
...
```{r results='hide'}
poro=read.table("soil-porosity.txt", col.names=c("A","B"))
```
...
```{r results='hide'}
maxdo = read.table("maximumDO.txt", header=TRUE)
maxdo
class(maxdo)
```
...
```{r fig.keep='none'}
boxplot(maxdo ~ location, data=maxdo,
        xlab="Location", ylab="mg/l")
```
...
\textbf{Reading data} & \\
\hline
\texttt{read.csv} & Read data from comma separated text file \\
\texttt{read.table} & Read data from general ASCII text file \\
\hline
\textbf{Working with data} & \\
\hline
...
\hline
\texttt{t.test} & T-test: both one sided (parameter $\mu$) and two sided (requires two vectors) \\
\texttt{var.test} & F-test: comparison of variance. Works with two vectors \\
\texttt{wilcox.test} & Wilcoxon rank test: comparison of medians. Both one sided (parameter $\mu$) and two sided (requires two vectors)  \\
\texttt{cor.test} & Significance test of correlation between two variables. Tests include Pearson's (default) and Spearman's  \\
\texttt{aov} & Analysis of variance --- uses formula syntax to define response variable and factors. Performs one-way and two-way ANOVA\\
\hline
\textbf{Other functions} & \\
\hline
\texttt{table} & Produces cross-tabulation of two different sets of categories \\
\texttt{as.matrix} & Converts object to matrix form \\
\texttt{cut} & Assign a vector of continuous values into defined bins \\
\hline
\end{tabularx}
...
### Crop yield dataset: *yield2.csv*
| Column header | Variable |
| --- | --- |
| Seed | Seed variety (A-C) |
| Fertilizer | Fertilizer treatment (1-5) |
| Yield | Crop yield |
