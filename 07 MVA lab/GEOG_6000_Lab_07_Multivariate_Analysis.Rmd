...
This lab is designed to cover the multivariate methods from the last two lectures, including cluster analysis and PCA. The following files will be used in this lab, all available on Canvas:
...
Both of these show fairly clear maximum values at around 3 groups, suggesting the $k=3$ maybe the best set of clusters for this data. But remember: **these are tools to help you decide how many clusters are needed, and your own judgement is as important**. For example, you may need a greater number of groups to understand the patterns in the data, or a suggested cluster may not have a useful explanation. 
...
R Command & Purpose \\
\hline
\multicolumn{2}{|l|}{\textbf{Cluster analysis}} \\
\hline
\texttt{dist} & Calculates distance between a set of multivariate observations (e.g. in a data frame. Returns triangular distance matrix with pairwise distance. Parameter ``method'' chooses distance metric\\
\texttt{hclust} & Performs hierarchical clustering. Takes a distance matrix as input. Parameter ``method'' chooses the method of cluster comparisons\\
\texttt{rect.hclust} & Adds rectangles to hierarchical cluster to delimit a set number of clusters. Parameter ``k'' chooses the cluster number\\
\texttt{cutree} & Cuts a hierarchical cluster classification into a set number of clusters. Parameter ``k'' chooses the cluster number\\
\texttt{kmeans} & Performs $k$-means cluster analysis. Parameter ``centers'' chooses the cluster number. Parameter ``nstart'' repeats the $k$-means analysis $nstart$ times, and returns the best solution\\
\texttt{calinhara} & Calculates the Calinski-Harabasz index for a cluster solution. Requires the \textbf{fpc} package\\
\texttt{silhouette} & Calculates the silhouette index for a cluster solution. Requires the \textbf{cluster} package\\
\hline
\multicolumn{2}{|l|}{\textbf{Principal component analysis}} \\
\hline
\texttt{prcomp} & Performs principal component analysis on a matrix or data frame using singular value decomposition. Parameter ``scale'' scales the data before analysis. See also \texttt{princomp} for eigenanalysis methods\\
\texttt{screeplot} & Produces a screeplot --- the eigenvalue for each component of a PCA\\
\texttt{biplot} & Produces a biplot of the output from a PCA. Parameter ``choices'' selects the components (axes) to plot\\
\hline
\multicolumn{2}{|l|}{\textbf{Other}} \\
\hline
\texttt{map} & Adds a simple map outline to a plot. Requires the \textbf{maps} package\\
\texttt{quilt.plot} & Function to quickly grid spatial point data for plotting\\
\texttt{rainbow} & Creates a color palette using a rainbow color scale\\
\texttt{scale} & Scales each column of a data frame. Default subtracts the mean and divides by the standard deviation to give $z$-scores (these can be changed). \\
\hline
