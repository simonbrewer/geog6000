...
- The iris dataset (morphological measurements on plants) *iris.csv*
- A dataset on world cities *cities.csv*
- A dataset of landcover types by elevation: *landcover.csv*
...
Before you do anything in R, it is important to set your working directory to where all your data and script files are or should be stored. Create a directory on the computer you are currently working on. Now change to this directory, either by typing `setwd("directoryname")` in the command window. For example:
...
Larger data sets will most commonly be read in from files. A recommended format to use are csv (comma-separated value) files, as these may be easily exchanged with spreadsheet software. Use the `read.csv()` function to read in the data from the Iris data file
```{r}
iris <- read.csv("iris.csv")
```

R will store data read in a data frame. To print out the contents of any variable, simply type the name of that variable at the command prompt. Other useful commands are `class()` to see what data class a variable is, and `names()` to get a list of the column headers. The function `str()` is probably the most useful, describing the column names and the type of data stored in them.
```{r results='hide'}
iris
class(iris)
names(iris)
...
#### The `tapply()` function
R has a set of `*apply` functions designed to help to do repetitive tasks. We will look at one of the simplest of these today, the `table` apply function. This *applies* a function (e.g. mean) to a vector, but calculates the function on subgroups defined by categories in a second vector. For example, to calculate the average sepal length per species of Iris, we can do the following:
```{r}
...
Restart R Studio (or R) and make sure you set the working directory to where your files are located. Now load the Iris and Cities datasets:
...
As we have only asked to plot one variable, this is represented on the y-axis. The x-axis gives the index of the observation, in this case from 1 to 150. Other index plots include strip plots and dot plots (also known as Cleveland plots). Here we use the first column of the cities data frame, which contains the city names, to label the plot.
```{r fig.keep='none'}
...
### Iris morphology dataset: *iris.csv* 
...
### World city dataset: *cities.csv* 
| Column header | Variable |
| --- | --- |
| City | City name |
| Area | Urban area |
| Pop.1980 | Population in 1980 (000s) |
| Pop.1990 | Population in 1990 (000s) |
| Pop.2000 | Population in 2000 (000s) |
| Growth |  |
| Food |  |
| PersRoom | |
| Water | Percentage access to water |
| Elec | Percentage access to electricity |
| Phones |  |
| Vehicles |  |

### Landcover/elevation dataset: *landcover.csv* 
| Column header | Variable |
| --- | --- |
| elevation | Elevation (m) |
| landcover | Landcover class (3) |
...
\hline
\textbf{R Command} & \textbf{Purpose} \\
\hline
\texttt{getwd} & Prints working directory \\
\texttt{setwd} & Changes working directory \\
\texttt{list.files} & Lists files in directory \\
\texttt{read.csv} & Reads data from CSV file \\
\texttt{help} or \texttt{?} & Get help on function \\
\texttt{q} or \texttt{quit} & Quits R session \\
\hline
\texttt{<-} or \texttt{=} & Attributes value to variable \\
\hline
\texttt{class} & Prints object class \\
\texttt{str} & Prints object structure \\
\texttt{summary} & Prints summary statistics \\
\texttt{table} & Prints counts of categories \\
\texttt{levels} & Prints category labels \\
\hline
\texttt{df\$column} & Access `column' from `df' dataframe \\
\texttt{df[,col]} & Access column `col' from `df' dataframe \\
\texttt{df[row,]} & Access row `row' from `df' dataframe \\
\texttt{df[row,col]} & Access value at row and column \\
\texttt{vec[id]} & Access value at position `id' in vector \\
\hline
\texttt{log} & Calculates natural logarithm \\
\texttt{log10} & Calculates log 10 \\
\texttt{round} & Round values to integer \\
\texttt{seq} & Generate sequence of numbers (from, to, by) \\
\texttt{sqrt} & Calculates square root \\
\texttt{sum} & Calculates sum of vector \\
\hline
\texttt{mean} & Calculates mean of vector \\
\texttt{median} & Calculates median of vector \\
\texttt{var} & Calculates variance of vector \\
\texttt{sd} & Calculates standard deviation of vector \\
\texttt{quantile} & Calculates quantile of vector (requires percentile parameter)\\
\texttt{cov} & Calculates covariance of two (or more) vectors\\
\texttt{cor} & Calculates correlation of two (or more) vectors\\
\hline
\texttt{plot} & Basic line and point plots \\
\texttt{stripchart} & 1D scatterplot \\
\texttt{dotplot} & Cleveland dot plot \\
\texttt{boxplot} & Boxplots \\
\texttt{hist} & Plot histogram \\
\texttt{qqnorm} & Plot quantile of vector against normal distribution \\
\texttt{qqline} & Plot line showing quantiles of two normal distributions \\
\hline
\end{tabularx}
