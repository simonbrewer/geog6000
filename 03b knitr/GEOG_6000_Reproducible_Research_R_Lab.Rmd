...
In RStudio, go to the 'File' menu > 'New File' > 'R Markdown'. A new markdown document will open in the top-left panel. This is a template and already has some markdown code in it, but we will start from scratch. So highlight and delete all the existing text to leave a blank document. 

We'll start by adding a header and a short introduction. Type the following code into the document:
...
If you look in your working directory, you should see two extra files with the same name as the R Markdown file, a simple markdown file with the extension *.md, and the html file (extension *.html). This second file can be open directly in a browser. 
