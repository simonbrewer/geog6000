...
If you haven't already done so, download the Cities data (*cities.csv*) and move it to your `datafiles` directory. Now read it in:
...
Restart R Studio (or R) and make sure you set the working directory to where your files are located. Now load the Penguin and Cities datasets:
...
```{r eval=FALSE}
legend("bottomleft",legend=c("Adelie","Chinstrap","Gentoo"),
       col=c(1,2,3), pch=16)
```
...
```{r fig.keep='none'}
boxplot(penguin$bill_length_mm ~ penguin$species, ylab='Bill length (mm)')
```
...
## penguin morphology dataset: *penguin.csv* 
