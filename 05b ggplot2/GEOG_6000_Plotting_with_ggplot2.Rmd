...
Now start RStudio and change the working directory to `lab05`. As a reminder, you can do this by going to the [Session] menu in RStudio, then [Change working directory]. This will open a file browser that you can use to browse through your computer and find the folder. Load these data sets, and use `str()` and `names()` to look at what they contain:
...
```{r fig.keep='none'}
qplot(bill_depth_mm, bill_length_mm, data = penguins,
      color = species, size = body_mass_g, alpha = I(0.7),
...
```{r fig.keep='none'}
qplot(bill_depth_mm, data = penguins, geom = 'histogram', binwidth = 0.25)
```

Or a  set of boxplots:
```{r fig.keep='none'}
qplot(species, bill_depth_mm, data = penguins, geom = 'boxplot')
```
