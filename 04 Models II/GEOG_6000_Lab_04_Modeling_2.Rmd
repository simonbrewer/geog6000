...
header-includes:
   - \usepackage{tabularx}
---
In this lab, we will look at how to build basic models in R, how to extend them to include some simple terms and how to perform some simple diagnostics. We will be using the following files for these examples:

- A dataset of runoff in Southern California and upstream snowfall from 6 sites: *SoCal_Runoffm.csv*
...
hist(irished$DVRT, main="DVRT", breaks=20,
     xlab="DVRT Scores", col='darkorange')
```

To run these commands, you can copy-paste them to the command window, or (in RStudio) you can run them directly by clicking on the line in the editor, then pressing Ctrl-R (Cmd-Return on Mac OSX). If you want to run all commands in the script, save the script from the file menu, giving it the name *plotIrish.r*. Now the script can be run using the `source()` command (or by clicking on the `Source' button in the top-right of the editor window).
...
```{r}
mySD(iris$Sepal.Length)  # compare to sd(sl)
...
```{r results='markup', fig.keep='none'}
runoff <- read.csv("SoCal_Runoff.csv")
summary(runoff)
pairs(runoff)
```
...
kidiq$mom.work2 = factor(kidiq$mom.work, 
       labels=c("unemployed","plumber","lawyer","doctor"))
kidiq.lm2 = lm(kid.score ~ mom.work2, data=kidiq)
summary(kidiq.lm2)
```

This returns 3 coefficients, for the plumber, lawyer and doctor categories. R uses the first factor specified as the reference category, so the results represent the offset in child IQ scores for these categories, relative to unemployed mothers. 

- What is the average IQ for children of plumbers? [Note: these labels were made up and should not be taken to reflect the real IQ of plumbers' children]
...
```{r eval=FALSE}
plot(kid.score ~ mom.iq2, data=kidiq, col=(kidiq$mom.hs+1), pch=16)
```
...
R Command & Purpose \\
\hline
\texttt{lm(y $\sim$ x)} & Model $y$ as a function of $x$. Note that the `$\sim$' dictates the nature of the relationship. Dependent variable(s) on the left hand side, independent on the right hand side \\
...
\section{Appendix: Making a VIF function}
\label{sec:vif}
\subsection{Making a function}
...
Now we'll add the loop to calculate the VIF. Remember that this is given by $1 / (1 - r^2)$ from a model between $x_i$ and all the other $x$s. Add the following code after the \texttt{names()} function:
```{r eval=FALSE}
  for (i in 1:nvars) {
    myy = x[,i]
    myx = x[,-i]
...
source("myvif.r")
```
Now save and source the file. Once you have done this, you can calculate the VIFs for the snowfall sites as follows:
```{r}
myvif(runoff[,-7])
```
